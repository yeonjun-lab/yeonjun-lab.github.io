...
prev_url: /foundations/languages/c/memory-model/concept/구조체는-메모리에-어떻게-배치되는가/
next_url: /foundations/languages/c/struct-layout/deep-dive/packed-struct는-왜-위험할-수-있는가/
layout: doc
section: foundations
...
## 7. 어디서부터 선언서의 직관과 실제 배치 검증이 갈리는가

### 7.1 구조체 정의를 읽는 것과 실제 레이아웃을 아는 것은 다른 문제다

필드 선언을 보면 대략적인 구조는 이해할 수 있다.  
하지만 실제 바이트 배치, padding, tail padding, 필드 시작 위치는  
그 선언만으로 확정해서 생각하면 위험할 수 있다.

즉, 구조체 선언서와 실제 layout 검증은 같은 단계가 아니다.

### 7.2 `sizeof` 하나만으로는 구조체 배치를 충분히 알 수 없다

전체 크기를 아는 것과  
각 필드가 어디서 시작하는지를 아는 것은 다른 정보다.

즉, `sizeof`는 전체 결과를 보여주고  
`offsetof`는 내부 경계를 보여준다.  
둘을 같이 봐야 구조체 배치가 읽힌다.

### 7.3 필드 순서를 바꾸는 일은 문서 순서를 바꾸는 것이 아니라 메모리 모델을 바꾸는 일이다

필드 선언 위치를 바꾸면  
단순 가독성만 달라지는 것이 아니라 실제 오프셋과 구조체 크기가 달라질 수 있다.

즉, 구조체 필드 재배치는 코드 정리 작업이 아니라  
배치 규칙 변경 작업일 수 있다.

### 7.4 layout 검증은 성능 문제와 호환성 문제에서 동시에 중요하다

대규모 배열에서는 메모리/캐시 문제로 중요하고,  
ABI/FFI/직렬화 경계에서는 정확성 문제로 중요하다.

즉, `offsetof`는 최적화 도구이면서 동시에  
호환성 검증 도구다.

### 7.5 추측이 맞더라도 검증을 생략할 이유는 되지 않는다

현재 플랫폼에서 예상한 결과가 나올 것 같아 보여도  
layout이 중요한 구조체라면 코드로 확인하는 습관이 더 안전하다.

즉, layout 문서는  
"감으로 이해하는 법"보다 "검증하는 법"을 목표로 해야 한다.

---

## 8. 실무에서 중요한 판단 기준

### 8.1 구조체 레이아웃이 중요한 경우 반드시 `sizeof`와 `offsetof`를 같이 본다
...
### 8.5 필드 순서를 바꿀 때는 의미/ABI/성능을 함께 검토한다
...
- 구조체 선언을 읽는 것과 실제 레이아웃을 검증하는 일을 분리해서 보고 있는가
- `sizeof`와 `offsetof`가 서로 다른 수준의 정보를 준다는 점을 이해하고 있는가
- 필드 순서 변경이 실제 메모리 모델 변경이라는 점을 의식하고 있는가
- 성능 문제와 호환성 문제 양쪽에서 layout 검증이 필요함을 보고 있는가
- 감이 맞더라도 중요한 구조체는 코드로 확인하고 있는가
