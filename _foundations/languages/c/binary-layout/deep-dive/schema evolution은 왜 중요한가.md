...
prev_url: /foundations/languages/c/binary-layout/deep-dive/tlv-type-length-value-는-왜-자주-쓰이는가/
next_url: /foundations/languages/c/binary-layout/deep-dive/버전-필드는-언제-필요하고-언제-해가-될-수-있는가/
layout: doc
section: foundations
...
## 7. 어디서부터 현재 포맷 편의와 장기 호환 계약이 갈리는가

### 7.1 지금 읽기 쉬운 포맷과 나중에도 안 깨지는 포맷은 다른 목표다

초기 구현에서는 가장 단순하고 compact한 포맷이 좋아 보일 수 있다.  
하지만 시간이 지나 필드가 늘고 reader/writer 버전이 어긋나면  
단기 단순성이 오히려 장기 비용이 될 수 있다.

즉, 현재 구현 편의와 장기 호환성은 같은 목표가 아니다.

### 7.2 필드 존재 여부보다 필드 의미 안정성이 더 중요할 수 있다

unknown field보다 더 위험한 것은  
같은 field id나 tag가 시간이 지나며 다른 의미를 가지는 경우다.

즉, schema evolution의 핵심은 필드 추가만이 아니라  
의미 계약을 얼마나 안정적으로 유지하느냐다.

### 7.3 compatibility는 포맷 구조만이 아니라 parser 정책 문제다

모르는 필드를 skip할지, 보존할지, 에러로 볼지에 따라  
실제 호환성 특성이 달라진다.

즉, schema evolution은 데이터 구조 설계와 함께  
reader/writer 정책 설계도 포함한다.

### 7.4 compact fixed format은 초기에는 좋아 보여도 미래 비용을 숨길 수 있다

확장 포인트가 전혀 없는 고정 포맷은  
처음에는 단순하지만 변화가 시작되면 전체 교체 비용이 커질 수 있다.

즉, evolution 친화성은 공간 오버헤드가 아니라  
미래 변경 비용을 줄이는 장치로 봐야 한다.

### 7.5 버전 번호 하나로 모든 호환성 문제가 해결되지는 않는다

버전 필드를 두더라도  
필드 추가/삭제/의미 변경 정책이 불명확하면 호환성은 여전히 취약하다.

즉, 버전 표시는 schema evolution의 일부일 뿐  
전체 해법은 아니다.
...
### 8.3 삭제보다 deprecate를 먼저 고려한다

실무에서는 필드를 바로 없애기보다  
deprecated 상태로 유지하면서 점진적으로 사용 중단하는 전략이 더 안전하다.

### 8.4 unknown field 처리 정책을 명확히 문서화한다
...
### 8.5 versioning 전략을 schema와 함께 설계한다
...
- 현재 구현 편의와 장기 호환성을 같은 목표로 보고 있지 않은가
- field id/tag 재사용과 의미 변경이 가장 위험한 변경 중 하나임을 의식하고 있는가
- compatibility를 포맷 구조뿐 아니라 parser 정책 문제로 보고 있는가
- compact fixed format의 미래 변경 비용을 고려하고 있는가
- 버전 필드만으로 호환성이 해결된다고 생각하고 있지 않은가

---

## 10. 더 깊게 볼 포인트

### 10.1 explicit version field vs implicit extensibility

포맷에 버전 필드를 둘지, TLV/optional field만으로 확장을 처리할지 비교할 수 있다.

### 10.2 field reservation strategy

미래를 위해 type range나 field id를 예약하는 설계로 확장할 수 있다.

### 10.3 unknown field preservation

읽은 unknown field를 다시 쓸 때 보존하는 전략이 왜 중요한지 더 깊게 볼 수 있다.

### 10.4 enum evolution rules
